...
public class Rotation : MonoBehaviour
{
    ...
    public int TspinCheck()
    {
        ...
        for (int x = 1; x >= -1; x -= 2)
        {
            for (int y = 1; y >= -1; y -= 2)
            {
                if (board.ActiveMinoCheckForTspin((int)Mathf.Round(gameStatus.ActiveMino.transform.position.x) + x, (int)Mathf.Round(gameStatus.ActiveMino.transform.position.y) + y))
                {
